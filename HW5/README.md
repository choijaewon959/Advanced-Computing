...
For time series processing, the naive moving average recomputes each window from scratch, resulting in `O(nw)` complexity. The optimized SIMD implementation uses ARM NEON on Apple Silicon to process two double values at once. This reduced runtime from 66 microseconds to 30 microseconds, giving around a 2.2x speedup. This shows how vectorization can improve throughput for repeated numerical operations such as moving averages, volatility estimates, signal calculations, and market data feature generation.
